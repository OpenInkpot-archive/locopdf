...
init_database(char *filename)
{
    ...
    sqlite3_exec(locopdf_database,
                 "CREATE TABLE files(fileid INTEGER PRIMARY KEY,filename TEXT UNIQUE,mod_time INTEGER)",
                 NULL, NULL, NULL);
    sqlite3_exec(locopdf_database,
                 "CREATE TABLE settings(fileid INTEGER,settingname TEXT,value TEXT, UNIQUE(fileid, settingname))",
                 NULL, NULL, NULL);
    return 0;
}

int
get_file_record_status(char *filename)
{
    ...
    char *temp =
        sqlite3_mprintf
        ("SELECT mod_time FROM files WHERE filename = \'%q\'", filename);
    if (sqlite3_get_table
        (locopdf_database, temp, &resultp, &rows, &cols,
         NULL) != SQLITE_OK) {
        sqlite3_free(temp);
...
int
update_file_mod_time(char *filename)
{
    struct stat filestat;
    int fileexists;
    fileexists = stat(filename, &filestat);
    ...
    if (rows == 0) {
        temp =
            sqlite3_mprintf
            ("INSERT INTO files (filename,mod_time) VALUES(\'%q\',%d)",
        ...
        temp =
            sqlite3_mprintf
            ("UPDATE files SET mod_time=%d WHERE filename=\'%q\'",
             filestat.st_mtime, filename);
        sqlite3_exec(locopdf_database, temp, NULL, NULL, NULL);
        sqlite3_free(temp);
    }
    sqlite3_free_table(resultp);
...
long
get_file_index(char *filename, int create_entry_if_missing)
{

    char **resultp = NULL;
    int rows, cols;
    char *temp =
        sqlite3_mprintf("SELECT fileid FROM files WHERE filename = \'%q\'",
                        filename);
    int result =
        sqlite3_get_table(locopdf_database, temp, &resultp, &rows, &cols,
                          NULL);
    sqlite3_free(temp);
    if (rows <= 0 && create_entry_if_missing) {
        ...
        char *temp =
            sqlite3_mprintf
            ("SELECT fileid FROM files WHERE filename = \'%q\'", filename);
        result =
            sqlite3_get_table(locopdf_database, temp, &resultp, &rows,
                              &cols, NULL);
        sqlite3_free(temp);
...
void
set_setting(char *filename, char *settingname, char *value)
{
    long fileindex = get_file_index(filename, 1);
    char *temp =
        sqlite3_mprintf
        ("INSERT OR REPLACE INTO settings (fileid,settingname,value) VALUES (%d,\'%q\',\'%q\')",
         fileindex, settingname, value);
    sqlite3_exec(locopdf_database, temp, NULL, NULL, NULL);
    sqlite3_free(temp);
}

void
set_setting_INT(char *filename, char *settingname, int value)
{
    char *tempstr;
...
void
set_setting_DOUBLE(char *filename, char *settingname, double value)
{
    char *tempstr;
...
char *
get_setting(char *filename, char *settingname)
{
    ...
    char *temp =
        sqlite3_mprintf
        ("SELECT value FROM settings WHERE fileid = %d AND settingname = \'%q\'",
         fileindex, settingname);
    sqlite3_get_table(locopdf_database, temp, &resultp, &rows, &cols,
                      NULL);
    sqlite3_free(temp);
    if (rows <= 0) {
...
int
get_setting_INT(char *filename, char *settingname)
{
...
double
get_setting_DOUBLE(char *filename, char *settingname)
{
    char *tempstr = get_setting(filename, settingname);
    if (tempstr == NULL)
        return -1;
